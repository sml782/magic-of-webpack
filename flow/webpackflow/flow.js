...
class Compiler {
  ...
  run() {
    ...
    const entryModule = { id: entry, source: entrySource };
    modules.push(entryModule);
    // 再找出该模块依赖的模块，再递归本步骤直到所有入口依赖的文件都经过了本步骤的处理
    const title = path.join(this.options.context, './src/title.js');
    const titleContent = fs.readFileSync(title, 'utf8');
    const titleSource = babelLoader(titleContent);
    const titleModule = { id: title, source: titleSource };
